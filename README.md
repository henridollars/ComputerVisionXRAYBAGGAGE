...
1. **Multi-method region proposals**: Combines multi-level thresholding (OTSU + 7 fixed + adaptive), Canny edge detection (6 configs), morphological top-hat/black-hat (3 scales), and MSER blob detection. Achieves 99.7% recall on validation.
...
4. **Post-processing**: Confidence gate (>0.5) after doing a threshold sweep, per-class size filter, per-class NMS, cross-class NMS, max 3 detections/image.

## Results

![Validation Score Distribution](val_distribution.png)

![Best Cases](best_cases.png)

![Worst Cases](worst_cases.png)
